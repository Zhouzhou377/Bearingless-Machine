...
static command_help_t cmd_help[NUM_HELP_ENTRIES] = {
		...
		{"disable_vctrl", "Disable bp3 velocity control"},
		{"disable_levctrl", "Disable bp3 levitation control"},
		...
		{"disable_ctrl", "Disable bp3 regulation"}
...
int cmd_bp3(int argc, char **argv)
{
	...
	if (strcmp("enable_vctrl", argv[1]) == 0) {
		// Check correct number of arguments
		if (argc != 2) return CMD_INVALID_ARGUMENTS;
		bp3_control_data.bp3_v_control.enable = 1;
		//reset_bp3();
		return CMD_SUCCESS;
	}
	...
	if (strcmp("enable_ob", argv[1]) == 0) {
		// Check correct number of arguments
		if (argc != 2) return CMD_INVALID_ARGUMENTS;
		bp3_control_data.bp3_v_control.para_ob.enable = 1;
		//reset_bp3();
		return CMD_SUCCESS;
	}
	...
	if (strcmp("set_ixy_ref", argv[1]) == 0) {
		...
		bp3_control_data.bp3_lev_control.Ixy0_ref[0] = ix_ref;
		bp3_control_data.bp3_lev_control.Ixy0_ref[1] = iy_ref;
	...
	if (strcmp("set_Fxy_ref", argv[1]) == 0) {
		...
		bp3_control_data.bp3_lev_control.F_xy[0] = Fx_ref;
		bp3_control_data.bp3_lev_control.F_xy[1] = Fy_ref;
	...
	if (strcmp("set_theta_offset", argv[1]) == 0) {
		...
		bp3_control_data.BP3_PARA->para_machine.theta_offset_xy = theta_offset;
	...
	if (strcmp("set_id", argv[1]) == 0) {
		...
		bp3_control_data.bp3_v_control.Idq0_ref[0] = id;
		return CMD_SUCCESS;
	}
	...
	if (strcmp("set_Te", argv[1]) == 0) {
		...
		bp3_control_data.bp3_v_control.Te_ref = Te;
		return CMD_SUCCESS;
	}
	...
	if (strcmp("set_injection_Fxy", argv[1]) == 0) {
		...
		bp3_control_data.bp3_lev_control.w_inject = w;
		bp3_control_data.bp3_lev_control.mag_inject = mag;
		return CMD_SUCCESS;
	}
