...
int cmd_cabinet(int argc, char **argv)
{
	...
	if (strcmp("pv3", argv[1]) == 0) {
		...
		set_pole_volts_three_phase(Va, Vb, Vc, inv);
	...
	if (strcmp("phv3", argv[1]) == 0) {
		...
		set_line_volts_three_phase(Va, Vb, Vc, inv);
	...
	if (strcmp("read_adc_3", argv[1]) == 0) {
		...
		double a = get_adc(inv->HW->sensor.Ia.adcCh);
		double b = get_adc(inv->HW->sensor.Ib.adcCh);
		double c = get_adc(inv->HW->sensor.Ic.adcCh);
